...
export class CartService {
  private cart = new BehaviorSubject<ItemCart[]>(null); //Definimos nuestro BehaviorSubject, este debe tener un valor inicial siempre Para guardar
  public currentDataCart$ = this.cart.asObservable(); //Tenemos un observable con el valor actual del BehaviorSubject Notificar un cambio
  public qtyItems = new Subject<number>();//No va a tener una variable que le de seguimientp, va a ser un metodo
  constructor() {//Dice que la variable siempre va a estar en el localStorage
    ...
    this.currentDataCart$ = this.cart.asObservable(); //Establece en automatico que el carrito se actualiza
  }
  saveCart(): void { //Dice que guarde lo que tenga en el carrito de compras en el localStorage y la reemplaza
    localStorage.setItem('orden', JSON.stringify(this.cart.getValue()));
  }
  addToCart(producto: any) {//Agrega un elemento nuevo o su cantidad
    ...
    newItem.idItem = producto.idProducto | producto.idItem;
  ...
  public removeFromCart(newData: ItemCart) {
    ...
    if (objIndex != -1) {
      ...
      listCart.splice(objIndex, 1); //Corre el elemente eliminando el espacio vacio en el carrito
    }
    this.cart.next(listCart); //Enviamos el valor al Observable
    //Actualizar la cantidad total de items del carrito
    this.qtyItems.next(this.quantityItems());
